...
import type { TipiImportacao } from '../types';
...
export const TipiImportModal: React.FC<Props> = ({ isOpen, onClose, onSuccess }) => {
    const [file, setFile] = useState<File | null>(null);
    const [vigencia, setVigencia] = useState<string>('');
    const [loading, setLoading] = useState(false);
    const [result, setResult] = useState<TipiImportacao | null>(null);
    ...
    const handleImport = async () => {
        ...
        if (!vigencia) {
            setError('Selecione uma data de vigência.');
            return;
        }
        setLoading(true);
        setError(null);
        ...
        try {
            const importResult = await tipiApi.importar(file, vigencia);
            if (importResult.status === 'ERRO') {
                throw new Error('Ocorreu um erro no processamento do arquivo no servidor.');
            }
            setResult(importResult);
        } catch (err: any) {
            const errMsg = err.response?.data?.detail || err.message || 'Erro ao processar o arquivo.';
            setError(errMsg);
        } finally {
            setLoading(false);
        }
            ...
            >
                <div className="p-6 border-b border-border-subtle flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-bold text-text-primary">Importar Tabela TIPI (Excel)</h2>
                    </div>
                    ...
                    {!result ? (
                        <div className="space-y-6">
                            ...
                            <div className="space-y-2">
                                <label htmlFor="vigencia" className="text-sm font-semibold text-text-primary">
                                    Data de Vigência
                                </label>
                                <input
                                    type="date"
                                    ...
                                    className="w-full px-4 py-2.5 bg-bg-deep border border-border-subtle rounded-xl text-text-primary focus:outline-none focus:border-brand-primary"
                                    required
                                />
                            </div>
                            ...
                            <label className="border-2 border-dashed border-border-subtle rounded-xl p-10 flex flex-col items-center gap-4 bg-bg-deep/30 transition-colors hover:bg-bg-deep/50 relative cursor-pointer group">
                                ...
                                <div className="text-center">
                                    <p className="text-sm font-semibold text-text-primary">
                                        {file ? file.name : 'Selecione a planilha Excel da TIPI'}
                                    </p>
                                    <p className="text-xs text-text-muted mt-1">
                                        Apenas formatos .xlsx e .xls são aceitos
                                    </p>
                                </div>
                            </label>
                            ...
                            <div className="flex justify-end gap-3 pt-2">
                                ...
                                <button
                                    disabled={!file || !vigencia || loading}
                        ...
                        <div className="space-y-8 py-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="p-4 bg-bg-deep rounded-xl border border-border-subtle">
                                    <p className="text-xs font-bold text-text-muted uppercase mb-1">Total Linhas</p>
                                    <p className="text-2xl font-display font-bold text-text-primary">{result.total_linhas}</p>
                                </div>
                                <div className="p-4 bg-green-500/5 rounded-xl border border-green-500/10">
                                    <p className="text-xs font-bold text-green-600 uppercase mb-1">Importadas</p>
                                    <p className="text-2xl font-display font-bold text-green-600">{result.total_importados}</p>
                                </div>
                                <div className="p-4 bg-yellow-500/5 rounded-xl border border-yellow-500/10">
                                    <p className="text-xs font-bold text-yellow-600 uppercase mb-1">Ignoradas</p>
                                    <p className="text-2xl font-display font-bold text-yellow-600">{result.total_ignorados}</p>
                                </div>
                                <div className="p-4 bg-red-500/5 rounded-xl border border-red-500/10">
                                    <p className="text-xs font-bold text-red-600 uppercase mb-1">Erros</p>
                                    <p className="text-2xl font-display font-bold text-red-600">{result.total_erros}</p>
                                </div>
                            </div>
