...
const SupplierForm: React.FC = () => {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                if (id) {
                    setLoading(true);
                    const res = await api.get(`/cadastro/fornecedores/${id}`);
                    const supplier = res.data;
                    setFormData(supplier);
                    if (supplier.state_id) {
                        fetchCities(supplier.state_id);
                    }
                }
            } catch (err) {
                console.error(err);
        ...
        try {
            ...
            if (state) {
                try {
                    ...
                    // 1. Try mapping by CEP first (IBGE match)
            ...
            alert('Falha ao consultar CNPJ. Verifique a conexão ou tente novamente.');
        } finally {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (id) {
                await api.put(`/cadastro/fornecedores/${id}`, formData);
            } else {
                await api.post('/cadastro/fornecedores', formData);
            }
            navigate('/cadastros/fornecedores');
        } catch (err: any) {
            const msg = err.response?.data?.detail || 'Erro ao salvar fornecedor.';
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-display font-bold text-text-primary tracking-tight">
                            {id ? 'Editar' : 'Novo'} <span className="text-brand-primary">Fornecedor</span>
                        </h1>
                        <p className="text-text-muted mt-1">Dados cadastrais simplificados.</p>
                    </div>
                </div>
