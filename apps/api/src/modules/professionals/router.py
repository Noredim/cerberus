...
def get_professionals(db: Session = Depends(get_db), current_user: User = Depends(get_current_user), company_id: str = Depends(get_active_company)):
    ...
    ).filter(Professional.tenant_id == current_user.tenant_id)
...
def get_available_users(db: Session = Depends(get_db), current_user: User = Depends(get_current_user), company_id: str = Depends(get_active_company)):
    ...
    # Se company_id for passado, consideramos limitados apenas os profissionais dessa company?
    # Usualmente o usuario tem apenas um professional (independente de company ou atrelado a company). Assumimos isolamento total.
    sub_query = db.query(Professional.user_id).filter(
        Professional.tenant_id == current_user.tenant_id,
        Professional.user_id.isnot(None)
    )
    if company_id:
        sub_query = sub_query.filter(Professional.company_id == company_id)
        
    bounded_user_ids = sub_query.all()
